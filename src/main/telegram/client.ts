...
// Telegram API credentials — register at https://my.telegram.org
...
export async function getOrCreateClient(accountId: number): Promise<TelegramClient> {
  ...
  const account = accountQueries.get(accountId) as any
...
export async function verifyPassword(phone: string, password: string): Promise<void> {
  ...
  const passwordCheck = await computeCheck(passwordInfo as any, password)
  const result = await pending.client.invoke(new Api.auth.CheckPassword({ password: passwordCheck }))

  await finalizeAuth(phone, pending.client, result)
}

async function finalizeAuth(phone: string, client: TelegramClient, _authResult: any): Promise<void> {
  ...
  const account = (accountQueries.list() as any[]).find((a: any) => a.phone === phone)
...
export async function syncAccount(accountId: number, win?: BrowserWindow): Promise<void> {
  ...
  for (const dialog of dialogs) {
    ...
    activities.push({
      tgId,
      lastReadDate: dialog.dialog?.readOutboxMaxId ? Math.floor(Date.now() / 1000) : null,
      lastMessageDate: (dialog.message as any)?.date ?? null,
...
export async function leaveEntity(accountId: number, entityDbId: number): Promise<void> {
  ...
  const tgEntity = await client.getEntity(entity.username || entity.tg_id)

  if (entity.type === 'channel' || entity.type === 'supergroup') {
    await client.invoke(new Api.channels.LeaveChannel({ channel: tgEntity }))
  } else if (entity.type === 'group') {
    await client.invoke(
      new Api.messages.DeleteChatUser({ chatId: entity.tg_id, userId: 'me' })
    )
  }
...
export async function transferEntities(
  ...
  for (const dbId of entityDbIds) {
    try {
      ...
      // skip private groups that can't be auto-joined
    }
    done++
    onProgress(done)
  }
